<template>
	...
	>
		...
		<p>
			<strong class="red">fetch-method</strong>
			属性设置获取服务端数据的接口函数，该函数返回Promise
		</p>
		<p>
		...
		<p>
			定义表格的列时，如果将某一列设置为可过滤字段，那么访问后端接口时会同时传输过滤参数。例如，设置username字段为可过滤，则传递给后端API的参数为：username=xx&limit=10&page=1&offset=0
		</p>
		<p>
			通过
			<strong class="red">extra-query</strong>
			属性可传递除分页、过滤以外的其他参数，组件监听该属性有变换时会重新请求数据。
		</p>
		<p>参数： {{ params }}</p>
		<admin-table-page
			:columns="columns"
		...
		<p>
			组件默认服务端API返回数据对象的key为{ total, items},
			如果服务端以其他key返回数据，可设置
			<strong class="red">items-key</strong>,
			<strong class="red">total-key</strong> 两个属性做映射。
		</p>
